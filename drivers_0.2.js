...
var { writeLog, writeFile, appendFile, formatDate, orderSettlementsByMonth } = require('./utils.js');
...
function excelSaveResume(resumeData, filePath, globalConfig) {
    ...
    try {
        ...
        writeLog(`Error 400: ${error}`, "ERROR", globalConfig);
        result.statusCode = 400;
...
function excelSaveCmeGroup(data, filePath, globalConfig) {
    ...
    try {
        ...
        var dataInfo = {
            date: settlementsOrder[0].tradeDate,
        ...
        dataLineTmp.push(resumeLineTmp);
        fs.readFile(TEMPLATE_PATH, function (err, templateData) {
            if (err) {
                const nowErrorEnd = new Date();
                const formattedErrorDateEnd = formatDate(nowErrorEnd.toLocaleString());
            ...
            var templateTmp = new xlsxTemplate(templateData);
            // Insert data into the template
            templateTmp.substitute("CU", { dataLineTmp: dataLineTmp });
            // Get binary data
            var binaryData = templateTmp.generate({ type: 'nodebuffer' });
            // Delete the file if exits
            if (fs.existsSync(TEMPLATE_TMP_PATH)) {
